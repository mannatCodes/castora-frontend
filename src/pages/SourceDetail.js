...
const SourceDetail = () => {
      ...
      return (
         <div className="max-w-4xl mx-auto">
            ...
            <button
               ...
               className="text-gray-300 hover:text-amber-300 flex items-center transition-colors duration-200"
            >
               <svg
   ...
   return (
      <div className="max-w-4xl mx-auto">
         <button
            onClick={handleGoBack}
            className="text-gray-300 hover:text-amber-300 flex items-center mb-6 transition-colors duration-200 group"
         >
            <svg
               xmlns="http://www.w3.org/2000/svg"
         ...
         <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-4">
            <h1 className="text-1xl font-medium text-gray-100 flex items-center">
               <div className="h-10 w-10 mr-3 flex items-center justify-center rounded-full bg-gradient-to-r from-stone-900 to-zinc-950 border border-gray-700 text-amber-400">
                  {source.name.substring(0, 2).toUpperCase()}
               </div>
               {source.name}
               {source.is_active ? (
                  <span className="ml-3 px-2 py-1 text-xs rounded-sm bg-gradient-to-r from-amber-900 to-amber-800 text-amber-300 border border-amber-700">
                     Active
                  </span>
               ) : (
                  <span className="ml-3 px-2 py-1 text-xs rounded-sm bg-gradient-to-r from-stone-900 to-zinc-950 text-gray-400 border border-gray-700">
                     Inactive
                  </span>
               )}
            </h1>
               ...
               <button
                  ...
                  className={`px-3 py-1.5 text-sm rounded-sm transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-900 ${
                     source.is_active
                        ...
                        : 'bg-gradient-to-r from-amber-700 to-amber-800 text-white border border-amber-600 hover:from-amber-600 hover:to-amber-700 focus:ring-amber-500'
                  }`}
               >
         ...
         <div className="bg-gradient-to-br from-stone-900 to-zinc-950 shadow-lg rounded-sm overflow-hidden border border-gray-700 mb-6">
                        ...
                        {sourceCategories && sourceCategories.length > 0 ? (
                           <div className="flex flex-wrap gap-2">
                              {sourceCategories.map((cat, index) => (
                                 <span
                                    key={index}
                                    className="inline-flex items-center px-2 py-1 rounded-sm text-xs font-medium bg-gradient-to-r from-zinc-950 to-stone-900 text-amber-300 border border-gray-700"
                                 >
                                    {cat}
                                 </span>
                              ))}
            ...
            <div className="p-6">
               <div className="flex justify-between items-center mb-4 border-b border-gray-700 pb-2">
                  ...
                  <button
                     ...
                     className="px-3 py-1.5 text-sm rounded-sm bg-gradient-to-r from-amber-700 to-amber-800 text-white border border-amber-600 hover:from-amber-600 hover:to-amber-700 transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-amber-500 focus:ring-offset-2 focus:ring-offset-gray-900 flex items-center"
                  >
                     <svg
                        xmlns="http://www.w3.org/2000/svg"
               ...
               {!source.feeds || source.feeds.length === 0 ? (
                  <div className="text-center py-8">
                     ...
                     <button
                        onClick={() => setIsAddFeedModalOpen(true)}
                        className="mt-2 text-amber-400 hover:text-amber-300 transition-colors duration-200"
                     >
                        Add a new feed
                     </button>
                  </div>
                        ...
                        <tbody className="divide-y divide-gray-700">
                           {source.feeds.map(feed => (
                              ...
                              >
                                 ...
                                 <td className="px-6 py-4 whitespace-nowrap">
                                    {feed.is_active ? (
                                       <span className="px-2 py-1 inline-flex text-xs leading-5 font-medium rounded-sm bg-gradient-to-r from-amber-900 to-amber-800 text-amber-300 border border-amber-700">
                                          Active
                                       </span>
                                    ) : (
                                       <span className="px-2 py-1 inline-flex text-xs leading-5 font-medium rounded-sm bg-gradient-to-r from-zinc-950 to-stone-900 text-gray-400 border border-gray-700">
                                          Inactive
                                       </span>
                                    )}
                                 </td>
         ...
         {isAddFeedModalOpen && (
            <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center p-4 z-50">
               <div className="bg-gradient-to-br from-stone-900 to-zinc-950 rounded-sm max-w-md w-full p-6 shadow-2xl border border-gray-700 relative">
                  <button
                  ...
                  <form onSubmit={handleAddFeed}>
                     ...
                     <div className="mb-4 flex items-center">
                        <input
                           ...
                           className="h-4 w-4 text-amber-600 bg-gray-900 border-gray-700 rounded focus:ring-amber-500 focus:ring-offset-gray-900"
                        />
                     ...
                     <div className="mt-6 flex justify-end space-x-3">
                        ...
                        <button
                           type="submit"
                           className="px-4 py-2 bg-gradient-to-r from-amber-700 to-amber-800 hover:from-amber-600 hover:to-amber-700 text-white rounded-sm focus:outline-none focus:ring-2 focus:ring-amber-500 focus:ring-offset-2 focus:ring-offset-gray-900 text-sm transition-colors duration-200"
                        >
                           Add Feed
                        </button>
                     </div>
