...
const Articles = () => {
   ...
   const [latestCrawledDate, setLatestCrawledDate] = useState(null);
   ...
   useEffect(() => {
      const fetchArticles = async () => {
         ...
         try {
            ...
            setLatestPublishedDate(data.latest_published_date || null);
            setLatestCrawledDate(data.latest_crawled_date || null);
         } catch (err) {
         ...
         {loading ? (
            ...
            <div className="grid grid-cols-1 gap-4">
               {articles.map((article, index) => (
                  <div
                     key={article.id}
                     className="bg-gradient-to-br from-stone-900 to-zinc-950 shadow-lg hover:shadow-xl transition-all duration-300 rounded-sm border-t border-gray-700 transform hover:scale-[1.01]"
                     onMouseEnter={() => setHoveredCard(article.id)}
                     onMouseLeave={() => setHoveredCard(null)}
                  >
                     <div className="h-0.5 w-full bg-gradient-to-r from-transparent via-amber-800 to-transparent opacity-60"></div>
                     {index % 2 === 1 && (
                        <div className="absolute top-0 left-0 w-1 h-full bg-gradient-to-b from-amber-400 via-amber-600 to-transparent"></div>
                     )}
                     <Link
                        ...
                        <div className="flex flex-col sm:flex-row sm:items-start">
                           <div className="flex-1">
                              <div className="flex items-center text-xs text-gray-400 mb-2">
                                 <span className="px-2 py-1 bg-gradient-to-r from-zinc-950 to-stone-900 text-amber-300 rounded-sm font-medium">
                                    {article.source_name}
                                 </span>
