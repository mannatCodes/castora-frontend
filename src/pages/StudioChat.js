...
const PodcastSession = () => {
   ...
   const handleChatResponse = responseData => {
      if (responseData.session_state) {
         updateSessionState(responseData.session_state);
      }

      if (responseData.is_processing === false) {
         setIsProcessing(false);
         setProcessingType(null);
         setCurrentTaskId(null);
      ...
      setIsProcessing(true);
      startPollingForCompletion(responseData.task_id);
   };
         ...
         if (pollCount > maxPolls) {
            ...
            setMessages(prev => [
               ...prev,
               {
                  role: 'assistant',
                  content:
                     'This is taking longer than expected. The podcast may still be processing in the background; please refresh this session in a moment.',
               },
            ]);
            return;
         }
   ...
   return (
      <div className="min-h-screen flex bg-[#0A0E14]">
         ...
         <div
            className={`fixed md:sticky top-0 left-0 h-full md:h-screen w-72 bg-gradient-to-b from-stone-900 to-zinc-950 border-r border-gray-700 z-30 transform transition-transform duration-300 ease-in-out ${sidebarClass}`}
         >
            <Sidebar
                     ...
                     <div className="hidden sm:flex items-center px-3 py-1 bg-[#121824] rounded-full border border-gray-700 text-xs text-gray-300">
                        <span
                           className={`w-2 h-2 rounded-full ${
                              isProcessing ? 'bg-amber-400 animate-pulse' : 'bg-gray-500'
                           } mr-2`}
                        ></span>
                        <span>
                           {isProcessing
            ...
            <main className="flex-1 flex flex-col max-h-[calc(100vh-4rem)] overflow-hidden">
               {showFinalPresentation ? (
                  ...
                  <>
                     ...
                     >
                        {isProcessing && (
                           <div className="mb-4 px-4 py-3 bg-amber-500/10 border border-amber-500/30 text-amber-400 text-sm rounded-md shadow-lg">
                              <div className="flex items-center">
                                 ...
                                 <input
                                    ...
                                    className={`w-full bg-[#121824] text-white border ${
                                       isProcessing || loading
                                          ? 'border-gray-600'
                                          : 'border-gray-700'
                                    } rounded-md py-3 pl-4 pr-12 focus:outline-none focus:ring-2 focus:ring-amber-500/50 focus:border-amber-600 placeholder-gray-500 ${
                                       isProcessing || loading
                                 ...
                                 <button
                                    ...
                                    className={`absolute right-2 h-9 w-9 flex items-center justify-center bg-gradient-to-r ${
                                       !inputMessage.trim() || isProcessing || loading
                                          ...
                                          : 'from-amber-700 to-amber-800 hover:from-amber-600 hover:to-amber-700 hover:shadow-md'
         ...
         {showCompletionModal && (
            <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4">
               <div className="bg-gradient-to-br from-stone-900 to-zinc-950 border border-gray-700 rounded-md max-w-md w-full p-6 text-center shadow-2xl animate-fade-in-up">
                  <div className="relative w-16 h-16 rounded-full bg-amber-500/20 flex items-center justify-center mx-auto mb-5">
                     <svg
                     ...
                     <div className="absolute inset-0 rounded-full bg-amber-500 opacity-10 animate-ping"></div>
                  </div>
                  ...
                  <button
                     ...
                     className="w-full py-3 bg-gradient-to-r from-amber-700 to-amber-800 hover:from-amber-600 hover:to-amber-700 text-white font-medium rounded-md transition-all flex items-center justify-center shadow-lg hover:shadow-amber-900/30 focus:outline-none focus:ring-2 focus:ring-amber-500 focus:ring-offset-2 focus:ring-offset-gray-900"
                  >
