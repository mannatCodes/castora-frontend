...
const SourceIcon = ({ url }) => {
   ...
   const defaultIconSvg = (
      <ExternalLink className="w-4 h-4 text-amber-400 transition-transform duration-200 group-hover:scale-110" />
   );
      ...
      <div className="flex -space-x-2 mr-2 relative">
         ...
         {displaySources.map((source, index) => {
            ...
            return (
               ...
               >
                  <div className="absolute inset-0 bg-amber-500/0 group-hover:bg-amber-500/10 transition-all duration-200"></div>
         ...
         {sources.length > maxIcons && (
            <div
               className="w-6 h-6 rounded-full bg-gradient-to-br from-amber-600/30 to-sky-600/30 border border-amber-500/30 flex items-center justify-center text-xs font-medium text-amber-400"
               style={{ zIndex: 0 }}
            >
               +{sources.length - maxIcons}
            </div>
...
const PodcastDetail = () => {
   ...
   useEffect(() => {
      const fetchPodcast = async () => {
         ...
         setAudioError(null);
         try {
            const response = await apiService.podcasts.getById(identifier);
   ...
   const handleTitleUpdate = async () => {
      ...
      try {
         ...
         const refreshedData = await apiService.podcasts.getById(identifier);
      ...
      return (
         <div className="min-h-screen flex items-center justify-center">
            <div className="text-center">
               <div className="w-12 h-12 border-3 border-amber-600 border-t-transparent rounded-full animate-spin mx-auto mb-3"></div>
   ...
   if (error) {
      return (
         <div className="min-h-screen flex items-center justify-center p-4">
            <div className="bg-gradient-to-br from-stone-900 to-zinc-950 border-l-4 border-red-500 p-4 rounded-sm shadow-sm mb-4 text-red-400">
               {error}
            </div>
         </div>
      );
   ...
   if (!podcast) {
      return (
         <div className="min-h-screen flex items-center justify-center bg-gray-900 p-4">
            <div className="bg-gradient-to-br from-stone-900 to-zinc-950 border-l-4 border-yellow-500 p-4 rounded-sm shadow-sm mb-4 text-yellow-300">
               Podcast not found.
            </div>
         </div>
      );
   ...
   return (
      <div className="min-h-screen py-4 px-4 relative overflow-hidden">
         ...
         <div className="absolute inset-0 opacity-20">
            ...
            <div
               className="absolute bottom-20 right-20 w-96 h-96 bg-sky-500/10 rounded-full blur-3xl animate-pulse"
         ...
         <div className="max-w-lg mx-auto relative z-10">
            <button
               ...
               className="text-gray-300 hover:text-amber-300 flex items-center mb-3 transition-colors duration-200 group"
            >
               <svg
            ...
            <div className="bg-gradient-to-br from-gray-900/80 via-stone-900/80 to-gray-800/80 rounded-2xl overflow-hidden shadow-2xl border border-gray-700/50 transition-all duration-300 hover:shadow-3xl backdrop-blur-lg">
               {hasBannerCarousel && (
               ...
               >
                  <div className="absolute inset-0 bg-gradient-to-r from-amber-600/5 to-sky-600/5" />
                  <div className="relative flex justify-between items-center">
                     <div className="flex items-center gap-3 min-w-0 flex-1">
                        <div
                           className={`p-1.5 bg-gradient-to-br from-amber-500/20 to-sky-500/20 rounded-lg transition-all duration-300 ${
                              isPlaying ? 'scale-110 shadow-lg shadow-amber-500/25' : ''
                           }`}
                        >
                           <Volume2
                     ...
                     <div className="relative">
                        <div className="absolute inset-0 overflow-hidden rounded-lg">
                           <div className="flex items-end justify-center h-full gap-px p-2">
                              {waveform.map((height, index) => (
                                 <div
                                    key={index}
                                    className={`bg-gradient-to-t from-amber-600/30 to-sky-400/30 rounded-full transition-all duration-300 ${
                        ...
                        {isPlaying && (
                           <div className="absolute inset-0 rounded-lg pointer-events-none">
                              <div className="absolute inset-0 border border-amber-500/20 rounded-lg animate-ping" />
                              <div className="absolute inset-1 border border-amber-400/10 rounded-lg animate-pulse" />
                           </div>
                        )}
                     </div>
               ...
               <div className="px-4 py-3 bg-gradient-to-r from-gray-900/50 to-gray-800/50 backdrop-blur border-t border-gray-700/30">
                  <div className="flex justify-center gap-3">
                     {hasScript && (
                        ...
                        >
                           <div className="absolute inset-0 bg-amber-500/0 group-hover:bg-amber-500/10 transition-all duration-200"></div>
                           <div className="p-1 bg-gradient-to-br from-amber-500/20 to-sky-500/20 rounded-full flex items-center justify-center group-hover:scale-110 transition-transform duration-200">
               ...
               {isPlaying && hasAudio && (
                  <div className="absolute top-0 left-0 w-full h-full pointer-events-none overflow-hidden rounded-2xl">
                     <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-64 h-64">
                        <div
                           className="absolute inset-0 border border-amber-500/10 rounded-full animate-ping"
                           style={{ animationDuration: '3s' }}
                        />
                        <div
                           className="absolute inset-8 border border-sky-400/10 rounded-full animate-ping"
                           style={{ animationDuration: '2s', animationDelay: '0.5s' }}
                        />
                        <div
                           className="absolute inset-16 border border-amber-300/10 rounded-full animate-ping"
            ...
            {isFullScriptOpen && hasScript && (
               <div className="fixed inset-0 z-50">
                  ...
                  <div className="absolute inset-0 flex justify-end">
                     <div
                        className={`w-full sm:w-96 bg-gradient-to-br from-gray-900/95 via-stone-900/95 to-gray-800/95 shadow-2xl border-l border-gray-700/50 backdrop-blur-xl flex flex-col transform transition-transform duration-300 ease-out ${
                        ...
                        <div className="px-4 py-3 bg-gradient-to-r from-gray-800/90 to-gray-700/90 border-b border-gray-700/30 backdrop-blur-sm flex-shrink-0">
                           ...
                           <div className="relative flex items-center justify-between">
                              <div className="flex items-center min-w-0 flex-1">
                                 <div className="p-1.5 bg-gradient-to-br from-amber-500/20 to-sky-500/20 rounded-lg mr-3 flex-shrink-0">
                                 ...
                                 >
                                    <div className="flex items-start">
                                       <div className="flex-shrink-0 pt-1">
                                          <div className="p-1.5 bg-gradient-to-br from-gray-800 to-gray-700 rounded-lg flex items-center justify-center group-hover:from-amber-500/10 group-hover:to-sky-500/10 transition-all duration-200">
                                             <SourceIcon url={sourceUrl} />
                                          </div>
                                       </div>
                                       <div className="ml-3 flex-1 min-w-0">
                                          <h3 className="font-medium text-amber-400 group-hover:text-amber-300 transition-colors truncate">
                                             {hostname}
                                          </h3>
                                          ...
                                          <div className="mt-2 flex items-center text-xs text-gray-500">
                                             <ExternalLink className="w-3 h-3 mr-1 text-amber-500/70 group-hover:translate-x-0.5 transition-transform duration-200" />
                  ...
                  <div className="bg-gray-900/95 backdrop-blur-xl border border-gray-700/50 rounded-2xl shadow-2xl max-w-md w-full p-8">
                     ...
                     <div className="mb-6">
                        ...
                        <input
                           ...
                           className="w-full px-4 py-3 bg-gray-800/50 border border-gray-600/50 rounded-xl text-gray-100 focus:outline-none focus:ring-2 focus:ring-amber-500/50 focus:border-amber-500/50 backdrop-blur-sm transition-all"
                     ...
                     <div className="flex justify-end space-x-3">
                        ...
                        <button
                           ...
                           className="px-6 py-3 bg-amber-600 text-white rounded-xl hover:bg-amber-700 flex items-center transition-all"
                           disabled={isSaving}
                        >
                           {isSaving ? (
                              <>
