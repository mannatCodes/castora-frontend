...
const ChatMessage = ({ message, role }) => {
   const formatMarkdown = content => {
      try {
         return content
            ...
            .replace(
               /• (.*?)(?:<br>|$)/g,
               '<div class="flex mt-1"><span class="mr-2 text-amber-400">•</span><span>$1</span></div>'
            );
      } catch (error) {
   ...
   return role === 'user' ? (
      <div className="mb-4 flex justify-end fade-in">
         <div className="max-w-[80%] bg-gradient-to-r from-amber-700 to-amber-800 text-white px-4 py-3 rounded-md rounded-tr-none text-sm shadow-md">
            {message}
         </div>
      </div>
   ) : (
      <div className="mb-4 flex fade-in">
         ...
         <div
            className="max-w-[80%] bg-gradient-to-r from-stone-900 to-zinc-950 text-gray-200 px-4 py-3 rounded-md rounded-tl-none text-sm shadow-md"
      ...
      <div className="my-2">
         <div className="bg-gray-900/60 backdrop-blur-md rounded-lg p-3 flex items-center relative overflow-hidden border border-white/10 shadow-lg">
            <div className="absolute inset-0">
               <div className="absolute top-0 left-0 w-full h-16 bg-gradient-to-b from-amber-500/10 to-transparent"></div>
               <div className="absolute -inset-1 bg-grid-pattern opacity-5"></div>
            </div>
            <div className="relative z-10 mr-3">
               <div className="relative">
                  <PonyoChatIcon size={32} isLoading={true} />
                  <div className="absolute inset-0 rounded-full border-2 border-amber-500/20 border-t-amber-400 animate-spin-slow"></div>
                  <div className="absolute -inset-1 bg-amber-400/10 rounded-full blur-sm animate-pulse-glow"></div>
               </div>
            </div>
            <div className="flex-1 flex flex-col z-10">
               ...
               <div className="h-1 w-full bg-gray-800/40 backdrop-blur-sm rounded-full overflow-hidden">
                  <div className="h-full rounded-full bg-gradient-to-r from-amber-400/80 to-amber-300/80 animate-loading-bar">
